...
Clients register webhooks in the Bruin portal: hamburger → Admin Center →
Integration → APIs & Webhooks → **Webhooks → Register Webhooks**. You cannot
register them via API. Direct the client to the portal / their CSE.
...
| `UnsuccessfulResolved` | The ticket was **canceled** by MetTel (wrong ticket type, duplicate). Closes all tasks; use it to prevent stale records in your system. |
...
| `Body.VendorId` | Present on `Created`, `NewNoteAdded`, `Resolved`, `Closed`, and `UnsuccessfulResolved` samples (not on `Unresolved`). |
...
- **`NewNoteAdded`**: `Body.NewNoteIds[]`, `Body.DetailIds[]`, `Body.VendorId`,
  and `Body.NoteMessages[]`, each note: `{ NoteID, NoteType, NoteValue,
  ServiceNumbers[], AttachmentExisted, EnteredDate }`. Many notes also carry
  `DetailID` (RFO, ResolutionTime, CLS, ReturnLabel); a plain `ADN` sample may
  omit it. Notes vary enormously; **filter on the `NoteType`s you care about.**
  Useful signals seen in the wild:
...
- **`Resolved` / `Closed`**: lightweight lifecycle payloads: `TicketId`,
  `Category`, `ClientId`, `VendorId`, `DetailIds[]`, `EnteredByUsername`.
- **`Unresolved`**: same shape without `VendorId` in the published sample.
- **`UnsuccessfulResolved`**: like Resolved but with `NewDetailId` (singular)
  instead of `DetailIds[]`.
